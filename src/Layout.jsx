    ...
    <header className="site-header">
      ...
      <div className={`mobile-panel ${open ? 'open' : ''}`} aria-hidden={!open}>
        ...
        <div className="mobile-panel-meta">
          <a href={`tel:${site.phone.replace(/[^0-9+]/g, '')}`}><Phone size={17}/> {site.phone}</a>
    ...
    <footer className="site-footer">
      <div className="shell footer-grid">
        ...
        <div>
          ...
          <div className="footer-links">{nav.slice(1, 7).map(([label, path]) => <Link key={path} to={path}>{label}</Link>)}</div>
        </div>
        <div>
...
function MobileActions() {
  return (
    <div className="mobile-actions" aria-label="Quick actions">
      <a href={`tel:${site.phone.replace(/[^0-9+]/g, '')}`}><Phone size={18}/><span>Call</span></a>
      <Link to="/contact"><MapPin size={18}/><span>Directions</span></Link>
      <Link to="/booking"><CalendarDays size={18}/><span>Reserve</span></Link>
    </div>
  )
}
